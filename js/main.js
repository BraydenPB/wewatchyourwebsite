...
(function () {
  ...
  function mobileMenu() {
    ...
    // Close if viewport grows past the mobile breakpoint (keep 1120px synced
    // with the CSS hide breakpoint where .mobile-menu is display:none and the
    // desktop nav appears — see the @media (min-width:1120px) blocks in
    // css/styles.css). If the menu were left open past it, the CSS would hide
    // the drawer while the page stays scroll-locked/inert.
    var mq = matchMedia("(min-width: 1120px)");
