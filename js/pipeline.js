...
(function () {
  ...
  // An interactive scanner (js/analyzer-scan.js) owns any [data-scan-driven]
  // host and drives its run on form submit — skip those here so the two don't
  // double-run. Plain [data-inspect] hosts (the home analyzer section) still
  // auto-run on scroll, exactly as before.
  var hosts = Array.prototype.filter.call(
    document.querySelectorAll("[data-inspect]"),
    function (h) { return !h.hasAttribute("data-scan-driven"); }
  );
