...
A fast, dependency-light static landing page for WeWatchYourWebsite. The home page
(`index.html`) plus a `/scanner/` Code Analyzer page (`scanner/index.html`) and two
placeholder legal pages, sharing one stylesheet and a handful of small vanilla-JS
modules that drive the canvas/WebGL "security console" visuals. No build framework,
no runtime backend — it serves as flat files and deploys to any static host.
...
Deploys are run manually with `npm run deploy` (above) — there is no CI/auto-deploy
workflow in this repo, so nothing ships until you run that command. If you'd rather
auto-deploy on push, you can add a GitHub Actions workflow that runs `npm run deploy`
with `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` as repository **secrets** (and an
optional `CLOUDFLARE_PROJECT` **variable**, non-sensitive, defaulting to `wwyw2`), or
connect the repo to Cloudflare Pages' native Git integration.
...
- **Allow the API origin in CSP.** `_headers` ships an **active** Content-Security-Policy
  with `connect-src 'self'`, so a cross-origin `fetch` to your scanner API will be blocked
  until you add its origin to `connect-src`. Fonts/scripts are already self-hosted, so
  `connect-src` is the only addition the scanner needs.
...
- [x] **Content-Security-Policy.** `_headers` ships an **active, enforcing** CSP alongside
      the other security headers (`Cross-Origin-Opener-Policy`, `nosniff`, `X-Frame-Options`,
      `Referrer-Policy`, `Permissions-Policy`). It was validated against every page (zero
      violations): `script-src 'self'` (no `'unsafe-eval'`), `style-src 'self' 'unsafe-inline'`
      (for the inline wordmark `<style>` and `style="--i:.."` attributes), self-hosted
      `font-src`/`img-src`, plus `object-src 'none'`, `base-uri`, `frame-ancestors`, and
      `form-action 'self'`. When you wire a real scanner backend, add its origin to
      `connect-src` (see "Wiring the analyzer"). `Strict-Transport-Security` (HSTS) is
      intentionally left off — enable it at the apex once the production domain is final.
