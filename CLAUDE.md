<!-- Keep in sync with AGENTS.md — Claude Code reads CLAUDE.md, Codex reads AGENTS.md. -->
# WeWatchYourWebsite — landing page

Static marketing site (home page + a `/scanner/` Code Analyzer page). **Framework-free, no build step**: flat HTML, one `css/styles.css`, a handful of vanilla-JS modules in `js/`. Ships as flat files to any static host (Cloudflare Pages). Goal: stay Lighthouse 100/100/100/100 (desktop + mobile), zero console errors, no runtime dependencies.
...
## Architecture
- `index.html` — the home/landing page; inline brand-SVG `<symbol>` sprite at top.
- `scanner/index.html` — the `/scanner/` page: the interactive Code Analyzer. Shares the home page's chrome (sprite/header/footer/`css/styles.css`, root-absolute asset paths). Driven by `js/analyzer-scan.js`; **demo today** — `requestScan()` is the single BACKEND SEAM where the client wires their real scanner (see README "Wiring the analyzer to a real backend"). The whole report renders from one `ScanResult` object.
- `css/styles.css` — all styles for both pages. Mobile-first. **CSS custom-property design tokens are the locked anchor — consume, never redefine inline.** No literal colors in components.
- `js/*.js` — self-contained ES5 IIFE/prototype modules that mount on `data-*` hooks (`data-glitch`, `data-console`, `data-netops`, `data-scan-radar`, `data-scanner-form`…). All loaded with `defer`. `js/vendor/lottie.min.js` is self-hosted (no CDN).
...
- The `/scanner/` page runs a **deterministic demo scan** (sample source + scripted findings) and submits nothing — this is intentional until the client wires `requestScan()` to their real scanner. The on-page "Demo" notice + the `data-scan-demo-note` line stay until then. Don't "make it real" client-side; the seam is the integration point.
